...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-2-push-arrives-on-locked-phone.md`
  ...
  constraint_from_story_1_2: Story 1.2 observed a send return `201` and never arrive — the device was off the network at the time, one minute after a reboot, and a later send arrived normally. The notification was not delayed; it was lost. **This outbox must therefore not treat `201` as delivered.** Delivery has to be acknowledged by the device or the user, and anything time-critical has to be re-sent until acted on, because a Failed Day that costs money must never be settled against a notification the author never saw. See `story-1-2-findings.md`, rows 2a and 2b.
