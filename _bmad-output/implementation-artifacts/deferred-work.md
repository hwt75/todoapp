...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "A Storage object is uploaded before its `evidence` row is inserted, so a refused insert leaves an object nothing references, with no cleanup path and no delete policy that could remove it."
...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "Deleting a commitment cascades its `evidence` rows away but leaves the Storage objects, which are then unreadable by anyone because the owner policy derives access from a parent row that no longer exists."
...
- source_spec: `_bmad-output/implementation-artifacts/spec-epic-6-retro-item-39-the-second-half-of-the-write-outlives-the-screen.md`
  summary: "The orphaned-object sweeper is now fully specified but still unbuilt — one mechanism serves both the stranded-upload entry above and the cascade-deleted-parent entry beside it."
