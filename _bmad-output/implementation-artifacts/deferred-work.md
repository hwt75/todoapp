...
## Deferred from: Epic 4 retrospective (2026-08-27)

- source_spec: `_bmad-output/implementation-artifacts/epic-4-retro-2026-08-27.md`
  summary: "Two independently machine-missed commitments on the same day: only the first Appeal can ever hold that day's one bundled Penalty (FR-13's per-day model). The second appeal attempt reads the trigger's raw exception text ('This day's Penalty was claimed by another appeal first.'), which is not a `23505` and so falls through `classifyWriteError`'s generic branch, with no distinguishable UX state and no retry-after-first-resolves path."
  evidence: Raised by the diff-scope review (adversarial lens). An author with two independently wrong machine-filed misses on the same day gets no clear path to ever contest the second one — the raw, undifferentiated error reads as a generic failure. Revisit by giving `appeal_hold_penalty()` a distinct, named exception for this case and having `AppealForm` special-case it with copy explaining that another commitment's appeal already holds the day's money.

- source_spec: `_bmad-output/implementation-artifacts/epic-4-retro-2026-08-27.md`
  summary: "`supabase/functions/pair-referee/index.ts`'s CORS header is a wildcard (`Access-Control-Allow-Origin: '*'`) on an endpoint that creates a privileged account and returns a plaintext one-time password in its response body. Separately, if that response never reaches the browser after the account row is already created (network drop, crash), there is no recovery path — `profile_single_referee`'s uniqueness permanently blocks creating another referee."
  evidence: Raised by the diff-scope review (adversarial lens). The CORS wildcard widens the attack surface for a token-bearing request from any origin (defense-in-depth gap, not directly exploitable without XSS); the missing recovery path is the more concrete risk — an ordinary flaky connection right after account creation permanently locks the doer out of ever pairing a working referee account. Revisit by restricting `Access-Control-Allow-Origin` to the app's own deployed origin, and adding a narrowly-scoped "resend/reset referee password" admin path.

- source_spec: `_bmad-output/implementation-artifacts/epic-4-retro-2026-08-27.md`
  summary: "The `appeal_evidence: file own` insert policy checks only `owner_id = auth.uid() and role_from_table() = 'doer'` — it never checks that the referenced appeal's own Penalty is still `held`. An author can attach new 'evidence' to an appeal that has already been ruled (`voided`/`owed`) or timed out (`dropped`)."
  evidence: Raised by the diff-scope review (adversarial lens). Low severity — there is no re-open flow, so a late upload is inert rather than harmful, just confusing to the author (false confidence that late evidence still matters when it structurally cannot change anything). Revisit by adding `and exists (select 1 from public.penalty p where p.id = (select penalty_id from public.appeal where id = appeal_id) and p.state = 'held')` to the insert policy's `with check`.
...
## Deferred from: code review of spec-6-8-a-photo-i-can-keep-against-any-commitment (2026-09-03)

- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "The author has no surface anywhere for opening a photo he has saved, so evidence is write-only to the one person it is kept for."
  evidence: Raised by all three review layers of Story 6.8. `createSignedUrl` is called in exactly one component, `components/referee-appeal-detail.tsx`, and it filters on `appeal_id` — so the referee can open an appeal's evidence and the doer can open nothing at all. `EVIDENCE_COPY.hint` says "It is private — only you can open it", and "you" is the one party with no route to it. Pre-existing since Story 6.3 for timed proofs; Story 6.8 makes it matter more, because a record the author deliberately keeps is worth less than a proof that only has to exist. Not fixed here: a viewer is a screen of its own, not a late addition, and the spec never promised one. Revisit as its own story — it is the natural next step after 6.8.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "RESOLVED 2026-09-07 by `spec-a-photo-that-proves-nothing-does-not-stay-forever.md` (migration `20260907130000`, function `evidence-sweeper`). A Storage object is uploaded before its `evidence` row is inserted, so a refused insert left an object nothing references, with no cleanup path that could remove it."
  evidence: Raised by the blind and edge-case layers of Story 6.8. `attachProof` in `components/today.tsx` uploads first and inserts second; every refusal the trigger can raise therefore strands the object. Pre-existing since Story 4.4 and unchanged in shape by 6.3 — but 6.8 widens the surface, since every photo-keeping commitment now has a folder of its own. No `storage.objects` delete policy exists for either the owner or the service role, so nothing can currently clean these up. Not fixed here: the honest repair is either a two-phase write or a sweeper, both larger than this story. Revisit alongside the viewer above, which is the first surface that would make an orphan visible.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "RESOLVED 2026-09-07 by the same sweeper. Deleting a commitment cascades its `evidence` rows away but leaves the Storage objects, which are then unreadable by anyone because the owner policy derives access from a parent row that no longer exists -- the sweeper now removes them an hour later, since nothing points at them any more, which is exactly the condition it tests for."
  evidence: Raised by the edge-case layer of Story 6.8. `evidence.commitment_id` is `on delete cascade`, and `storage.objects` has no corresponding trigger. The same shape already applies to `appeal` and `declaration` parents, so this is a generalization of an existing gap rather than a new one — recorded because the commitment parent is the first whose folder accumulates across many days rather than one. Revisit with the orphan sweeper above; the two want the same mechanism.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "Two photo-keeping commitments sharing a name render two identical upload labels, and `commitment.name` has no uniqueness constraint to prevent it."
  evidence: Raised by the edge-case layer of Story 6.8. The control is labelled from `row.name`, so a screen-reader user meets two controls announced identically with nothing to tell them apart. Real but narrow — it needs the author to deliberately name two commitments the same, and the whole product has one user. Not fixed here rather than reaching for a uniqueness constraint, which would be a product rule invented by a review. Revisit if a duplicate name ever actually occurs, or alongside any general pass over row labelling.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-8-a-photo-i-can-keep-against-any-commitment.md`
  summary: "`evidence_object_is_a_commitment_day()` is a `security definer` existence oracle: any authenticated account can ask whether an arbitrary uuid names a commitment belonging to anyone."
  evidence: Observed while reviewing the Story 6.8 referee narrowing. The function is granted to `authenticated` because a `storage.objects` policy has to call it, and it deliberately exposes no column — the answer is a bare boolean over a caller-supplied path. The leak is therefore "this uuid is some commitment", to a caller who had to guess a v4 uuid to ask. Not fixed here: the alternatives are worse (keying the policy on `evidence` would change what the referee can read about orphaned objects, which `4-6-the-referee-rules.sql` step 8 depends on), and the exposure is not distinguishable from random guessing. Revisit only if this product ever runs somewhere accounts are held by people the author did not choose.

## Deferred from: code review of spec-6-9-a-photo-i-can-open-again (2026-09-03)

- source_spec: `_bmad-output/implementation-artifacts/spec-6-9-a-photo-i-can-open-again.md`
  summary: "The Supabase storage mock is copied almost verbatim into four test files, so any change to the storage client's shape has to be made in four places."
  evidence: Raised by the blind layer of Story 6.9. `signedByPath` / `signed` / `createSignedUrl` appear in near-identical form in `lib/evidence.test.ts`, `components/chains-detail.test.tsx`, `components/focus-session.test.tsx` and `components/today.test.tsx`. Real, and the count only grows with each surface that reads a photo. Not fixed here because a shared test harness is its own piece of work and this story's own mocks are correct; the duplication is a maintenance cost, not a defect. Revisit alongside any fifth surface, or a general pass over the test harnesses.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-9-a-photo-i-can-open-again.md`
  summary: "On Chains detail the count of photos that could not be signed is reported once for the whole history, so the reader is told a photo failed without being told which day it belonged to."
  evidence: Raised by the blind layer of Story 6.9. Signed photos render inside the row of the day they belong to, but the failure count sits under the list as a single aggregate. Honest and never a shorter list, which is the rule the story actually set — but less useful than a per-day note would be. Not fixed here rather than reshaping the helper's return for a case that needs a signing failure to occur at all. Revisit if signing failures turn out to happen in practice.

## Deferred from: code review of spec-6-7-the-referee-may-object (2026-09-03)
...
## Deferred from: spec-epic-6-retro-item-38-a-queued-claim-keeps-the-meaning-it-had (2026-09-07)

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-6-retro-item-38-a-queued-claim-keeps-the-meaning-it-had.md`
  summary: "RESOLVED 2026-09-07 by `spec-a-time-switched-off-stays-off.md` (migration `20260907120000`). `due_time_as_of()` answered with the commitment's creation time for any day after its time was cleared, so a day that was genuinely untimed read as timed."
  evidence: The function's fallback is a `coalesce` of two subqueries (`20260829090000_midnight_decides_the_day.sql:238-252`, carried unchanged through item 38). When the log entry in force at day start is the one that *cleared* the time, that subquery returns null — the correct answer — and the coalesce falls through to the earliest entry, which is the value the commitment was created with. Its own comment says the creation value is a fallback "for any day at or before its own creation", so this is the implementation disagreeing with the documented rule, not the rule. The fix is an existence test rather than a coalesce, exactly as the new `late_window_at()` is written. Deliberately not made here: item 38's frozen boundary is that no settled day may be re-judged, and this changes which days settlement calls timed — it needs its own spec, its own regression over a cleared-then-later-day history, and a check of what it moves for any day already settled.

## Deferred from: spec-epic-6-retro-item-39-the-second-half-of-the-write-outlives-the-screen (2026-09-07)

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-6-retro-item-39-the-second-half-of-the-write-outlives-the-screen.md`
  summary: "RESOLVED 2026-09-07 by `spec-a-photo-that-proves-nothing-does-not-stay-forever.md` (migration `20260907130000`, function `evidence-sweeper`). The orphaned-object sweeper was fully specified but unbuilt — one mechanism serves both the stranded-upload entry above and the cascade-deleted-parent entry beside it."
  evidence: Item 39 closed the largest producer of orphans (leaving Today mid-upload no longer abandons the `evidence` insert) and defined the cleanup rather than building it, because a sweeper needs a `storage.objects` delete path that does not exist for any role today. The definition, in that spec's Design Notes: an orphan is an `appeal-evidence` object with no `evidence.storage_path` equal to its `name` and a `created_at` older than a one-hour grace period, which exists only to protect the gap between upload and insert; it must never delete an object whose row exists, one inside the grace period, or anything outside that bucket; and it would be a `security definer` function owned by `postgres` on its own `pg_cron` schedule, shaped like `outbox_schedule`, with a test staging a real orphan, a real in-grace object and a real referenced object. Remaining producers are the refusals `evidence_derive_owner()` raises after the object is already stored — a wrong capture date, a day that has ended, a parent the caller does not own.

## Deferred from: spec-epic-6-retro-item-42-a-row-with-no-photo-behind-it (2026-09-10)

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-6-retro-item-42-a-row-with-no-photo-behind-it.md`
  summary: "Nothing tells the maintainer whether the A1 hole was ever used: no diagnostic names the `evidence` rows already in the live project that have no object behind them."
  evidence: The new gate (`20260910090000`) is insert-only by design, so it cannot speak about rows filed before it existed — and the spec's own Ask First says so rather than guessing. A read-only function mirroring `orphaned_evidence_objects()` from the other side (`select e.id, e.storage_path from public.evidence e where e.swept_at is null and not exists (select 1 from storage.objects o where o.bucket_id = 'appeal-evidence' and o.name = e.storage_path)`) is about ten lines and answers it. It needs the maintainer's decision first: a row it names is either a fabricated one or an artefact of a sweep that stamped nothing, and only he can say what should happen to the days those rows are holding.

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-6-retro-item-42-a-row-with-no-photo-behind-it.md`
  summary: "A database refusal message reaches the author verbatim, in English, on a Vietnamese surface — this gate adds a sentence to a channel that was already open."
  evidence: `components/today.tsx` renders `insertError.message` as the failure reason, so every refusal `evidence_derive_owner()` raises (wrong capture date, a day that has ended, a parent not owned) has always surfaced as raw server English; `evidence_object_must_exist()` adds one more. Pre-existing and not caused by this change, but the new sentence is the least likely of them to ever be seen by an honest client — which is also why fixing it is a copy question (an `EVIDENCE_COPY` entry keyed off the failure, not the message) rather than a wording question.

## Deferred from: code review of spec-7-2-a-message-meant-to-be-said-out-loud (2026-09-10)

- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-a-message-meant-to-be-said-out-loud.md`
  summary: "CI's db-tests job cannot pass between 20:01 and 06:59 Asia/Ho_Chi_Minh: `supabase/tests/3-2-focus-prompt.sql` refuses to run outside 07:00-20:00 by design, and the job runs every file."
  evidence: PR #11's two db-tests runs failed at 21:05 local with that file's own guard (`the local hour is 21. Run it between 07:00 and 20:00 Asia/Ho_Chi_Minh`) after every earlier file had passed; PR #10, pushed at 20:40, passed. Not a defect in the test -- its steps subtract up to seven hours from `now()` and need headroom on both sides -- but the job fails on a change that touches no SQL, and the only remedy is re-running it in the morning. Options, in order of cost: skip that one file under an env flag CI sets outside the window; pin the session clock with a `set timezone` plus an injected hour for that file; or accept the window and say so in `.github/workflows` and `supabase/tests/README.md`. Needs hwt75's call on which.
  resolution: **Closed 2026-09-11 — hwt75 chose the first.** The window is declared by the file that has one (`-- ci-clock-window: 7-20 Asia/Ho_Chi_Minh`) and the `db-tests` loop skips that file outside it, by pattern rather than by filename, with a `::warning` and a run-summary section so a partial suite is never mistaken for a full one. What the option as written did not cover, and the implementation adds: `lib/sql-clock-window.test.ts` holds each marker to the `raise` its file really carries — hours and timezone both — and fails on a clock-bound file carrying no marker at all; and the job refuses outright when the runner cannot resolve the declared zone, because a shell without tzdata answers in UTC without saying so, which here is seven hours off and would skip the working day while running the file at midnight. The second option was not taken because injecting an hour would stop the file exercising the thing it exists to exercise — `enqueue_focus_prompts()` reading the real `now()`.

## Deferred from: spec-8-1-a-commitment-can-ask-for-the-referee-s-signature (2026-09-11)

- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-a-commitment-can-ask-for-the-referee-s-signature.md`
  summary: "A commitment of kind `do` with cadence `daily_hours_quota` can carry the referee sign-off flag, and no day it ever has can be signed off."
  evidence: `commitment_sign_off_needs_a_do` (20260911090000:60-62) constrains the kind and says nothing about the cadence, so `do` + `daily_hours_quota` + the flag is accepted by the database, by `canBeSignedOff()` and by the setup form. Nothing breaks -- `commitments_owing()` has excluded `daily_hours_quota` entirely since `20260820140000_weekly_quota_is_not_judged_daily.sql`, so such a commitment never produces a commitment-day, the referee's Story 8.4 list can never name one, and the flag is simply inert. What it does cost is a promise: the setup surface shows `REFEREE_SIGN_OFF_COPY.warning` and tells the author his friend can refuse this day, which for an hours quota is not true and never will be. Epic 8's own SPEC names the case in the rationale under its kind constraint (`SPEC.md:63`, "abstain and daily_hours_quota have no moment to photograph") while the rule beside it speaks only of kind, and story 8.1's frozen I/O matrix inherited the narrower reading -- the two disagree, and the implementation followed the matrix. Raised by the implementing agent, confirmed against the constraint, and put to hwt75 on 2026-09-11 with three options (add a fourth CHECK now, leave it, defer to 8.2). **hwt75 chose to leave it**, on the frozen matrix as approved. The fix, if it is ever wanted, is one CHECK on the cadence plus a second argument to `canBeSignedOff()`, a fourth refusal sentence and its form branch -- cheap, and cheapest of all inside Story 8.2, which is the next change to touch what a flagged day means.
  resolution: Half closed by Story 8.2 on 2026-09-14, on hwt75's decision recorded in that story's Spec Change Log. `sign_off_day()` refuses a REFUSAL on `daily_hours_quota` with its own sentence — without it the RPC would report success for a decision `commitments_owing()` excludes outright and can never read, which is worse than the inert flag this entry describes. Asserted in `supabase/tests/8-2-the-referee-s-decision-and-what-a-refusal-costs.sql` step 15, beside the other two landing guards. **Still open:** no CHECK was added, so the combination remains saveable, and `REFEREE_SIGN_OFF_COPY.warning` still tells the author his friend can refuse this day — which for an hours quota the database now guarantees he cannot. Story 8.2's frozen **Never** list excludes every client change, so the copy half was not this story's to fix. The remaining fix is the one this entry already names: one CHECK on the cadence plus a second argument to `canBeSignedOff()`, a fourth refusal sentence and its form branch.

## Deferred from: code review of spec-8-1-a-commitment-can-ask-for-the-referee-s-signature (2026-09-11)

- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-a-commitment-can-ask-for-the-referee-s-signature.md`
  summary: "`REFEREE_SIGN_OFF_COPY.warning` tells the author his referee can refuse the day before midnight, and until Story 8.2 ships there is no way for any referee to do that."
  evidence: Raised independently by the blind-hunter layer, which named it the A2 shape -- the app promising something the rules do not yet keep, which is the defect Story 8.1's own decision 3 exists to remove one instance of. It is real: story 8.1's `## Boundaries & Constraints` **Never** list is explicit that nothing reads the flag, there is no referee surface and no notification, so an author who switches sign-off on today gets a control, a warning and no mechanism. It differs from A2 in one way that matters -- A2's sentence was false and staying false, this one is false for as long as it takes 8.2 and 8.4 to land, and it fails toward the author (he believes his day is being watched more closely than it is, never less). hwt75 was given three options on 2026-09-11 -- reword now and reword again in 8.2, hide the control until 8.2, or accept and record -- and **chose to accept and record**, on the grounds that the epic ships incrementally and 8.2 is the next story. Close this when 8.2 makes the sentence true; if 8.2 slips or the epic is paused with 8.1 shipped, reopen it, because the argument for accepting it is entirely an argument about elapsed time.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-a-commitment-can-ask-for-the-referee-s-signature.md`
  summary: "The sentence explaining why a disabled control cannot be used is announced to nobody: it is a bare sibling paragraph with no `aria-describedby`, and a disabled checkbox is not focusable."
  evidence: Raised by the blind-hunter layer against the new sign-off control in `components/commitment-form.tsx`, where `signOffRefusal` renders as `<p className="row-muted">` after the label. Confirmed pre-existing rather than introduced: the Auto-check block (`commitment-form.tsx:274-294`) has had exactly this shape since Story 4.1, and `TIMED_COMMITMENT_COPY.warning` the same. So the new control copies the house pattern faithfully and the house pattern is the defect -- which is why it is recorded here rather than patched into one story's control and left inconsistent with its two neighbours. EXPERIENCE.md:155 requires that a check-like row be "disabled with an explanation", and an explanation a screen reader never reaches does not satisfy it. The fix is one `aria-describedby` per control plus ids on the three paragraphs, and it should be done to all three at once. `_bmad-output/planning-artifacts/ux-designs/ux-todoapp-2026-08-11/review-accessibility.md` is where it belongs in the planning record.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-a-commitment-can-ask-for-the-referee-s-signature.md`
  summary: "`supabase/tests/README.md`'s \"What is here\" table describes 26 of the directory's 46 files, so a reader cannot use it to find out what is covered."
  evidence: Surfaced by the blind-hunter layer while checking that story 8.1 had added its row -- it had, and the count line was correctly moved from 38 to 46 on the strength of a full local run. The gap is older and larger: the files from Epics 4 and 5, `6-7-the-referee-may-object.sql`, and every file named after a migration rather than a story (`carries_penalty_freezes_by_day.sql`, `one-live-doer.sql`, `every-surface-asks-the-day-the-same-question.sql`) appear nowhere in the table. Low consequence -- CI globs the directory, so nothing goes unrun, and no coverage is actually missing -- but the table is the only index of what the SQL suite proves, and a half-index reads as a complete one. Either finish it in one pass or replace it with a generated list; do not let it drift further one story at a time.

## Deferred from: review of spec-8-2-the-referee-s-decision-and-what-a-refusal-costs (2026-09-14)

- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-the-referee-s-decision-and-what-a-refusal-costs.md`
  summary: "The referee's existing day-lookup screen still offers an Object control on a flagged commitment, and the database now refuses it every time."
  evidence: Raised independently by the blind-hunter and verification-gap layers. `referee_day_lookup()` (`20260908180000:147-189`) returns `objection_deadline` and `already_objected` and knows nothing about `requires_referee_approval`, so `objectionIsOffered()` (`lib/referee.ts:526-532`) still returns true for a flagged commitment that froze `held`, and `components/referee-day-lookup.tsx:218` renders the button. Story 8.2's new `object_to_day()` guard then answers "That commitment asks for your signature on the day itself, not here." This is the client-mirror-stricter-or-looser-than-the-rule shape that Story 8.1's own review caught one layer up, and Epic 6 retrospective item 50 named. Not fixed in 8.2 because its frozen **Never** list excludes every UI change and hands the referee surfaces to Stories 8.3 and 8.4. The sentence the referee reads is at least informative rather than a raw error, which is why it was judged tolerable for the interval. Close it in 8.4, which rebuilds what the referee is offered; if 8.4 slips or the epic pauses with 8.2 shipped, reopen, because the whole argument for accepting it is an argument about elapsed time — the same argument, and the same shape, as 8.1's warning-before-the-mechanism entry above.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-the-referee-s-decision-and-what-a-refusal-costs.md`
  summary: "Between Story 8.2 and Story 8.5 a refusal charges the author 500,000 dong on a day he claimed held and photographed, and the reason is stored where nothing renders it."
  evidence: Raised by the blind-hunter layer as a product consequence rather than a defect. 8.2 stores `referee_decision.reason` verbatim and deliberately makes no outbox call; CAP-7 and the telling belong to Story 8.5, which is two stories away. `components/ledger.tsx` reads `objection`, not `referee_decision`, so the Ledger shows a failed day with a penalty and no explanation anywhere in the app. This matters more here than it would in most projects because this one ships each story straight to production with hwt75 as its only user, so the interval is real calendar time on a live account rather than a hypothetical. Accepted rather than fixed: folding the notification into 8.2 is exactly the merge that `.memlog.md:48` refuses, on the grounds that Story 6.7 did it and produced the 1,579-line single-block test file Epic 6 retrospective G2 named. The mitigation available without code is to not exercise a refusal on the live account until 8.5 lands. **Downgraded 2026-09-14 by hwt75:** no money is actually collected while the app is in development, so the penalty a refusal mints is bookkeeping rather than a loss and the mitigation this entry recommended -- not exercising a refusal on the live account until 8.5 -- no longer applies. What stays is the missing telling itself: a failed day with a stored reason that no surface renders is still a gap CAP-7 owns, and it becomes a real consequence again the moment collection is switched on. Close it in 8.5 either way.

- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-the-referee-s-decision-and-what-a-refusal-costs.md`
  summary: "`settle_day()` is the only writer of money-relevant state that does not take the per-account advisory lock, and a decision committing mid-run can leave a settlement internally inconsistent."
  evidence: Found by the blind-hunter layer at iteration 2 of Story 8.2's review and confirmed against the code. `settle_day()` (`20260829090000:397-556`) calls `commitments_owing()` in five separate statements under READ COMMITTED (`:449, 464, 473, 509, 520`), with the counts and the penalty insert (`:439-497`) running before the frozen-outcome write (`:502-509`). A `sign_off_day()` refusal committing between those two statements yields a settlement reading `clean` with `missed_count = 0` and no penalty row, while the frozen `settlement_commitment` row for that commitment reads `missed` — a broken chain with no penalty for a Grace Day to attach to, and `grace_day_validate()` (`20260825110000:158-180`) refuses it on both of its conditions at once. No later correction path knows how to repair that shape. Story 8.2 narrows the window by refusing a decision on an already-settled day, which is check-then-act and leaves the instant between the check and the insert. The real closure is the per-account key `pg_advisory_xact_lock(hashtext(owner_id::text))` that `grace_day_validate()` (`:156`), `appeal_hold_penalty()` (`20260825120000:54`), `mark_penalty_collected()` and `object_to_day()` (`20260906080451:40, :185`) all already take and `settle_day()` alone does not — which would close several older races of the same shape at once. Deliberately not done inside Story 8.2: it changes the most load-bearing function in the schema for a reason that predates this epic, and hwt75 chose narrow-and-record on 2026-09-14 rather than widen the story. Note this is the same *class* as epic-6-retro item 36, which added the lock to `object_to_day()` and `mark_penalty_collected()` but did not ask why the settler itself had none.
