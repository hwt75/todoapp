...
| `3-2-focus-prompt.sql`      | The hours-quota prompt is silent before its derived hour and the moment a target is met, but not for a still-unmet account-mate; a retried pass is a no-op; the four-slot bound holds; an archived commitment is never spoken for; the hour caps at 23 rather than wrapping into the next day.                                                                     |

**All twelve pass**, run on 2026-08-20 against a local stack with every migration applied from
